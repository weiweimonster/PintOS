...
tid_t thread_create(const char* name, int priority, thread_func* function, void* aux) {
  ...
  list_init(&(t->map_fd_files));
  list_init(&(t->unused_fd));
  t->curr_fd = 3;
  ...
  // Create a temp fpu to save current thread's fpu
  uint8_t temp[108];
  asm volatile("fsave (%0)" : : "g"(&temp) : "memory");

  /* Initialize FPU and save newly initialized FPU to switch thread frame's fpu member*/
  asm volatile("fninit":::"memory");
  asm volatile("fsave (%0)" : : "g"(&sf->fpu) : "memory");

  asm volatile("frstor (%0)" : : "g"(&temp) : "memory");
